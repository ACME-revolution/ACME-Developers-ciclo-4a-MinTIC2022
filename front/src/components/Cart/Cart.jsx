    ...
    return (
        <div className="cartContainer">
            <div
                ...
                className="buttonCartContainert">
            ...
            {cartItems && cartOpen && (
                <div className="cart">
                    <h2 className="cartMensaje">Tu carrito</h2>

                    {cartItems.lenght === 0 ? <p className="cartVacio">Tu carrito está vacio</p> : (
                        <div className="productsCart">{cartItems.map((item, i) => (
                            <ItemCart key={i} item={item} />
                        ))}
                        </div>
                    )}
                    <h2 className="total">Total: ${total} COP 
                    <button className='buttonsCart2' onClick={() => payProducts()}>Pagar</button> </h2> 
                </div>
            )}
        </div>
    )
